// jQuery-circleNav - v1.0

;(function ($) {

  // Plugin definition
  $.fn.circleNav = function (options) {

    // Default settings
    const settings = $.extend({}, $.fn['circleNav'].settings, options);

    // Plugin code
    return this.each(function () {
      const $container = $(this); /* Navigation container: #circle-nav-wrapper */
      const $toggle = $('.circle-nav-toggle');
      const $panel = $('.circle-nav-panel');
      const $menu = $('.circle-nav-menu');
      // breakpointChk();

      // Overlay
      if (settings.overlay) {
        if ($('.circle-nav-overlay').length === 0) {
          $('body').append("<div class='circle-nav-overlay'></div>");
          $(".circle-nav-overlay").css({
            "top": "0",
            "right": "0",
            "bottom": "0",
            "left": "0",
            "position": "fixed",
            "background-color": settings.overlayColor,
            "opacity": settings.overlayOpacity,
            "z-index": "-1",
            "display": "none"
          });
        }
      }

      // Toggle click event
      $('.circle-nav-toggle, .circle-nav-overlay').on('click tap', function () {
        $container.stop().toggleClass('circle-nav-open');
        $toggle.stop().toggleClass('circle-nav-open');
        $panel.stop().toggleClass('circle-nav-open');
        $menu.stop().toggleClass('circle-nav-open');
        $('.circle-nav-overlay').fadeToggle();

        if ($('body').css("overflow")) {
          $('body, html').css("overflow", "");
        } else {
          $('body, html').css("overflow", "hidden");
        }
      });
    });
  };

  // Default settings
  $.fn['circleNav'].settings = {
    overlay: true,
    overlayColor: "#fff",
    overlayOpacity: ".7"
  };

})(jQuery, window, document);

$(function () {
  $('#circle-nav-wrapper').circleNav();
  $('[data-bs-toggle="tooltip"]').tooltip();
  let open_menu = false;
  $('#circle-nav-toggle').on('click tap', function () {
    $('#circle-nav-wrapper').toggleClass(function () {
      open_menu = !open_menu;
      return 'd-none';
    });
  });
  let scroll_top = true;
  $(window).on("scroll", function () {
    if ($(this).scrollTop() <= 60 && !scroll_top) {
      $('#circle-nav-toggle').toggleClass('d-none');
      const elementCircleNavWrapper = $('#circle-nav-wrapper');
      elementCircleNavWrapper.removeClass('d-none');
      elementCircleNavWrapper.addClass('d-none');
      scroll_top = true;
    } else if ($(this).scrollTop() > 60 && scroll_top) {
      scroll_top = false;
      $('#circle-nav-toggle').toggleClass('d-none');
      if (open_menu) {
        $('#circle-nav-wrapper').toggleClass('d-none');
      }
    }
  });
});
